...
@Autonomous
public class EncoderIfBBlue  extends OpMode{
  ...
  @Override
    ...
    public void loop() {
        if(!trip1){
            rampUp(one*3.25, 0.0, 0.5, 0.5);
            trip1 = tripLoop(one*3.25);
        }
        /*else if(!trip2) {
            ...
            trip3 = tripLoopSideways(-one*2.5);
        }
        else if (!servoPosDown){
            moveArmDown();
        }
        else if (!trip4){
            rampUpSide(one*2.5, 0.0, 0.5, 0.5);
            trip4 = tripLoopSideways(one*1.5);
        }
        else if (!servoPosUp){
            moveArmUp();
        }
    ...
    /*private void twoBarLiftFast(){
        double twoBar = 0;
        twoBarLift.setPower(gamepad2.right_stick_y);
    }*/
